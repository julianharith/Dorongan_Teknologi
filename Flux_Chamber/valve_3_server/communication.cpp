...
void publishStatusAllServers(String data_kontrol, String data_status, String data_berita) {
  ...
  if (clientVPS.connected() && !isVPSConnecting) {
    ...
    innerObj["label_variabel"]   = "valve";
    innerObj["label_satuan"]     = "state";
...
void processIncomingCommand(String msg) {
  ...
  commandValue.trim();
  Serial.printf("\n[VALVE RECV] Menerima Perintah Kode: %s\n", commandValue.c_str());

  int targetCommand = commandValue.toInt();
  executeValveCommand(targetCommand);
}

void callbackVPS(char* topic, byte* payload, unsigned int length) {
  String msg = "";
  for (int i = 0; i < length; i++) msg += (char)payload[i];
  if (String(topic) == TOPIC_VPS_CMD_VALVE) processIncomingCommand(msg);
}

void callbackLocal(char* topic, byte* payload, unsigned int length) {
  String msg = "";
  for (int i = 0; i < length; i++) msg += (char)payload[i];
  if (String(topic) == TOPIC_LOCAL_CMD_VALVE) processIncomingCommand(msg);
}
    ...
    if (WiFi.status() == WL_CONNECTED && !clientVPS.connected()) {
      if (millis() - lastVPSTryTimer >= 10000) {
        ...
        if (WiFi.hostByName(VPS_HOST, dummyIP) == 1) {
          Serial.print("[VPS-TASK] Internet Aktif! Menghubungkan Valve ke VPS...");
